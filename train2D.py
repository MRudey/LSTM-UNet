...
if not tf.__version__.split(".")[0] == "2":
    raise ImportError(
        f"Required tensorflow version 2.x. current version is: {tf.__version__}"
    )
...
def train():
    ...
    with tf.device(device):
        ...
        model = params.net_model(
            params.net_kernel_params, params.data_format, False
        )

        # Losses and Metrics

        ce_loss = losses.WeightedCELoss(
            params.channel_axis + 1, params.class_weights
        )
        seg_measure = losses.seg_measure(
            params.channel_axis + 1, three_d=False
        )
        ...
        train_summary_writer = val_summary_writer = train_scalars_dict = (
            val_scalars_dict
        ) = None
            ...
            with tf.device("/cpu:0"):
                with writer.as_default():
                    for (
                        scalar_loss_name,
                        scalar_loss,
                    ) in scalar_loss_dict.items():
                        tf.summary.scalar(
                            scalar_loss_name, scalar_loss.result(), step=step
                        )
                    for image_name, image in images_dict.items():
                        if params.channel_axis == 1:
                            image = tf.transpose(image, (0, 2, 3, 1))
                        tf.summary.image(
                            image_name, image, max_outputs=1, step=step
                        )
        ...
        try:
            ...
            for _ in range(int(ckpt.step), params.num_iterations + 1):
                ...
                image_sequence, seg_sequence, _, is_last_batch = (
                    train_data_provider.get_batch()
                )
                if params.profile:
                    tf.summary.trace_on(graph=True, profiler=True)
                train_output_sequence, train_predictions, train_loss_value = (
                ...
                if not int(ckpt.step) % params.write_to_tb_interval:
                    if not params.dry_run:
                        seg_onehot = tf.one_hot(
                            tf.cast(
                                tf.squeeze(
                                    seg_sequence[:, -1], params.channel_axis
                                ),
                                tf.int32,
                            ),
                            depth=3,
                ...
                if (
                    int(ckpt.step) % params.save_checkpoint_iteration == 0
                    or int(ckpt.step) == params.num_iterations
                ):
                    if not params.dry_run:
                ...
                if not int(ckpt.step) % params.validation_interval:
                    ...
                    val_output_sequence, val_predictions, val_loss_value = (
                        val_step(val_image_sequence, val_seg_sequence)
                    )
                    model.reset_states_per_batch(
                        val_is_last_batch
                    )  # reset states for sequences that ended
                    if not params.dry_run:
                        ...
                        log_print(
                            "Printed Validation Step: {} to Tensorboard".format(
                                int(ckpt.step)
                            )
                        )
                    else:
...
if __name__ == "__main__":
    ...
    class AddDatasets(argparse.Action):
        def __init__(self, option_strings, dest, *args, **kwargs):
            super(AddDatasets, self).__init__(
                option_strings, dest, *args, **kwargs
            )

        def __call__(self, parser, namespace, values, option_string=None):
            if len(values) % 2:
                raise ValueError(
                    "dataset values should be of length 2*N where N is the number of datasets"
                )
            datastets = []
    ...
    arg_parser = argparse.ArgumentParser(
        description="Run Train LSTMUnet Segmentation"
    )
    ...
    arg_parser.add_argument(
        "--num_train_threads",
        dest="num_train_threads",
        type=int,
        help="Number of train data threads",
    )
    arg_parser.add_argument(
        "--num_val_threads",
        dest="num_val_threads",
        type=int,
    ...
    arg_parser.add_argument(
        "--unroll_len", dest="unroll_len", type=int, help="LSTM unroll length"
    )
    arg_parser.add_argument(
        "--num_iterations",
        dest="num_iterations",
    ...
    arg_parser.add_argument(
        ...
        dest="save_checkpoint_iteration",
        type=int,
        help="number of iterations between save checkpoint",
    )
    arg_parser.add_argument(
        "--save_checkpoint_max_to_keep",
        dest="save_checkpoint_max_to_keep",
        type=int,
        help="max recent checkpoints to keep",
    )
    arg_parser.add_argument(
        "--save_checkpoint_every_N_hours",
        dest="save_checkpoint_every_N_hours",
        type=int,
        help="keep checkpoint every N hours",
    )
    arg_parser.add_argument(
        "--write_to_tb_interval",
        dest="write_to_tb_interval",
        type=int,
    ...
    args_dict = {
        key: val for key, val in vars(input_args).items() if val is not None
    }
    print(args_dict)
