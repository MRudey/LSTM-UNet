...
from utils import bbox_crop, bbox_fill, get_model, log_print
...
def inference():
    # Load Model
    with open(
        os.path.join(params.model_path, "model_params.pickle"), "rb"
    ) as fobj:
        ...
        for T, image in enumerate(dataset):
            ...
            if not params.dry_run:
                ...
                seg_cell = np.logical_and(
                    np.equal(np.argmax(image_softmax_np, 0), 1).astype(
                        np.float32
                    ),
                ...
                if params.FOV:
                    ...
                    remove_ind = np.setdiff1d(
                        np.arange(num_cells), unique_fov_labels
                    )
                else:
                    remove_ind = []
                if params.save_intermediate:
                    ...
                    cv2.imwrite(
                        filename=out_fname, img=sigoutnp_vis.astype(np.uint16)
                    )
                ...
                for n in range(1, num_cells):
                    ...
                    if (
                        params.min_cell_size <= area <= params.max_cell_size
                        and n not in remove_ind
                    ):
                        p += 1
                ...
                cv2.imwrite(
                    filename=out_fname, img=labels_out.astype(np.uint16)
                )
                log_print("Saved File: {}".format(out_fname))
                if params.save_intermediate:
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(
        description="Run Inference LSTMUnet Segmentation"
    )
