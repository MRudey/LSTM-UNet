    ...
    def _override_params_(self, params_dict: dict):
        ...
        for key, val in params_dict.items():
            if key not in this_dict:
                print(
                    "Warning!: Parameter:{} not in defualt parameters".format(
                        key
                    )
                )
...
class CTCParams(ParamsBase):
    ...
    net_kernel_params = {
        "down_conv_kernels": [
            [
                (3, 128),
                (3, 128),
            ],  # [(kernel_size, num_filters), (kernel_size, num_filters), ...] As many convolustoins in each layer
            [(3, 256), (3, 256)],
            [(3, 256), (3, 256)],
            [(3, 512), (3, 512)],
        ],
        "lstm_kernels": [
            [
                (5, 128)
            ],  # [(kernel_size, num_filters), (kernel_size, num_filters), ...] As many C-LSTMs in each layer
            [(5, 256)],
            [(5, 256)],
            [(5, 512)],
        ],
    ...
    def __init__(self, params_dict):
        ...
        self.train_data_base_folders = [
            (os.path.join(ROOT_DATA_DIR, ds[0]), ds[1])
            for ds in self.train_sequence_list
        ]
        self.val_data_base_folders = [
            (os.path.join(ROOT_DATA_DIR, ds[0]), ds[1])
            for ds in self.val_sequence_list
        ]
        self.train_data_provider = self.data_provider_class(
            sequence_folder_list=self.train_data_base_folders,
        ...
        self.val_data_provider = self.data_provider_class(
            sequence_folder_list=self.val_data_base_folders,
            image_crop_size=self.crop_size,
            unroll_len=self.unroll_len,
            deal_with_end=0,
            batch_size=self.batch_size,
        ...
        if self.load_checkpoint and self.continue_run:
            if os.path.isdir(self.load_checkpoint_path):
                if self.load_checkpoint_path.endswith(
                    "tf-ckpt"
                ) or self.load_checkpoint_path.endswith("tf-ckpt/"):
                    self.experiment_log_dir = self.experiment_save_dir = (
                ...
                self.load_checkpoint_path = os.path.join(
                    self.load_checkpoint_path, "tf-ckpt"
                )
        else:
        ...
        if not self.dry_run:
            ...
            os.makedirs(
                os.path.join(self.experiment_log_dir, "train"), exist_ok=True
            )
            os.makedirs(
                os.path.join(self.experiment_log_dir, "val"), exist_ok=True
            )
    ...
    def __init__(self, params_dict: dict = None):
        ...
        if not self.dry_run:
            ...
            if self.save_intermediate:
                now_string = datetime.now().strftime("%Y-%m-%d_%H%M%S")
                self.save_intermediate_path = os.path.join(
                    self.save_intermediate_path,
                    "IntermediateImages",
                    now_string,
                )
                self.save_intermediate_vis_path = os.path.join(
                    self.save_intermediate_path, "Softmax"
                )
                self.save_intermediate_label_path = os.path.join(
                    self.save_intermediate_path, "Labels"
                )
