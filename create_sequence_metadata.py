...
def main(
    ...
    raw_file_template,
    seg_file_template,
    tra_file_template=None,
    force=False,
    FOV=80,
):
    if (
        os.path.exists(
            os.path.join(root_dir, "metadata_{}.pickle".format(seq))
        )
        and not force
    ):
        raise FileExistsError(
            "File {} already exists! use --force option to overwrite"
        )

    all_image_files = os.listdir(
        os.path.join(
            root_dir, os.path.join(os.path.dirname(raw_file_template))
        )
    )
    all_seg_files = os.listdir(
        os.path.join(
            root_dir, os.path.join(os.path.dirname(seg_file_template))
        )
    )
    all_tra_files = os.listdir(
        os.path.join(
            root_dir, os.path.join(os.path.dirname(tra_file_template))
        )
    )
    t = 0
    ...
    while True:
        if os.path.basename(raw_file_template.format(t)) in all_image_files:
            ...
            tra_fname = (
                tra_file_template.format(t)
                if tra_file_template is not None
                else None
            )
            ...
            if os.path.basename(seg_file_template.format(t)) in all_seg_files:
                ...
                if tra_fname and os.path.basename(tra_fname) in all_tra_files:
                    ...
                    dilation = grey_dilation(
                        seg.astype(np.int32), structure=strel.astype(np.int8)
                    )
                    seg_boundary = np.zeros_like(im, dtype=np.bool)
                    seg_boundary[
                        np.logical_and(
                            np.not_equal(seg, dilation),
                            np.greater(dilation, 0),
                        )
                    ] = True
                    ...
                    plt.pause(0.1)
                    valid_seg = input(
                        "Frame {}: Are all cells in the frame annotated [Y/n/yes to all]? ".format(
                            t
                        )
                    ).lower()
        ...
        t += 1
    with open(
        os.path.join(root_dir, "metadata_{}.pickle".format(seq)), "wb"
    ) as f:
...
def get_default_run():
    ...
    seg_file_template = os.path.join(
        "{}_GT".format(seq), "SEG", "man_seg{:03d}.tif"
    )
    tra_file_template = os.path.join(
        "{}_GT".format(seq), "TRA", "man_track{:03d}.tif"
    )  # Optional
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--force",
        dest="force",
        help="Force overwrite existing metadata pickle",
    )
    sys_args = sys.argv
    ...
    if len(sys_args) > 1:
        if sys_args < 5:
            raise SyntaxError(
                "Please input all parameters: root_dir, raw_file_template, seg_file_"
