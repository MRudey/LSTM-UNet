...
class WeightedCELoss(object):
    ...
    def __call__(self, gt_sequence, output_sequence):
        ...
        pixel_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=gt_sequence, logits=output_sequence
        )
        weighted_pixel_loss = pixel_loss * pixel_weights * gt_sequence_valid
        loss = tf.reduce_sum(weighted_pixel_loss) / (
            tf.reduce_sum(gt_sequence_valid) + 0.00001
        )
        return loss
...
def seg_measure(channel_axis, three_d=False, foreground_class_index=1):
    ...
    def connencted_components(input_np):
        ...
        for d1, images in enumerate(input_np):
            for d2, image in enumerate(images):
                labeled_image, max_num_temp = ndimage.label(
                    image, structure=strel
                )
            ...
            for gt, seg in zip(gt1, seg1):
                for this_label in np.unique(gt):
                    ...
                    for s in np.unique(overlaping_inds):
                        if s == 0:
                            continue
                        intersection = np.sum(overlaping_inds == s).astype(
                            np.float32
                        )
    ...
    def calc_seg(gt_sequence, output_sequence):
        with tf.device("/cpu:0"):
            ...
            gt_fg = tf.equal(
                tf.cast(gt_sequence, tf.float32), foreground_class_index
            )
            output_classes = tf.argmax(output_sequence, axis=channel_axis)
            output_foreground = tf.equal(
                output_classes, foreground_class_index
            )
            seg_measure_value = tf.py_function(
                seg_numpy, inp=[gt_fg, output_foreground], Tout=[tf.float32]
            )
...
def seg_measure_unit_test():
    ...
    calc_seg_meas = seg_measure(
        channel_axis=channel_axis, three_d=three_d, foreground_class_index=1
    )
    h = w = d = 30
    batch_size = 3
    unroll_len = 2
    if three_d:
        gt_sequence = np.zeros((batch_size, unroll_len, d, h, w, 1)).astype(
            np.float32
        )
        output_sequence = np.zeros(
            (batch_size, unroll_len, d, h, w, 3)
        ).astype(np.float32)
        output_sequence[:, :, :, :, 0] = 0.25
    else:
        gt_sequence = np.zeros((batch_size, unroll_len, h, w, 1)).astype(
            np.float32
        )
        output_sequence = np.zeros((batch_size, unroll_len, h, w, 3)).astype(
            np.float32
        )
        output_sequence[:, :, :, :, 0] = 0.25
    ...
    for b in range(batch_size):
        for u in range(unroll_len):
            for obj_id, (xs, xe, ys, ye) in enumerate(objects):
                gt_sequence[b, u, ys + i : ye + i, xs + i : xe + i] = (
                    obj_id + 1
                )
                output_sequence[
                    b,
                    u,
                    max(ys + i + 2, 0) : max(ye + i, 0),
                    max(xs + i, 0) : max(xe + i, 0),
                    1,
                ] = 0.5
            i += 1
