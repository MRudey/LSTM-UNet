...
class DownBlock2D(k.Model):
    def __init__(
        ...
        lstm_kernels: List[tuple],
        stride=2,
        data_format="NCHW",
    ):
        super(DownBlock2D, self).__init__()
        data_format_keras = (
            "channels_first" if data_format[1] == "C" else "channels_last"
        )
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        for conv_layer, bn_layer, lrelu_layer in zip(
            self.Conv, self.BN, self.LReLU
        ):
            conv = conv_layer(activ)
            bn = bn_layer(conv, training)
            activ = lrelu_layer(bn)
        out_shape = activ.shape
        activ_down = tf.reshape(
            activ,
            [
                orig_shape[0],
                orig_shape[1],
    ...
    def reset_states_per_batch(self, is_last_batch):
        ...
        for convlstm_layer in self.ConvLSTM:
            ...
            new_states = (
                cur_state[0] * is_last_batch,
                cur_state[1] * is_last_batch,
            )
...
class UpBlock2D(k.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        input_sequence = k.backend.resize_images(
            input_sequence,
            self.up_factor,
            self.up_factor,
            self.data_format_keras,
        ...
        input_tensor = tf.concat(
            [input_sequence, skip], axis=self.channel_axis
        )
        for conv_layer, bn_layer, lrelu_layer in zip(
            self.Conv, self.BN, self.LReLU
        ):
    ...
    def unit_test(cls):
        ...
        for i in range(4):
            ...
            skip = np.random.randn(
                batch_size * unroll_len, h * 2, w * 2, d
            ).astype(np.float32)
...
class ULSTMnet2D(k.Model):
    def __init__(
        ...
        if not len(net_params["down_conv_kernels"]) == len(
            net_params["lstm_kernels"]
        ):
            raise ValueError(
                "Number of layers in down path ({}) do not match number of LSTM layers ({})".format(
                    len(net_params["down_conv_kernels"]),
                    len(net_params["lstm_kernels"]),
                )
            )
        if not len(net_params["down_conv_kernels"]) == len(
            net_params["up_conv_kernels"]
        ):
            raise ValueError(
                "Number of layers in down path ({}) do not match number of layers in up path ({})".format(
                    len(net_params["down_conv_kernels"]),
                    len(net_params["up_conv_kernels"]),
                )
            )

        for layer_ind, (conv_filters, lstm_filters) in enumerate(
            zip(net_params["down_conv_kernels"], net_params["lstm_kernels"])
        ):
            stride = (
                2
                if layer_ind < len(net_params["down_conv_kernels"]) - 1
                else 1
            )
            self.DownLayers.append(
                DownBlock2D(conv_filters, lstm_filters, stride, data_format)
            )
            self.total_stride *= self.DownLayers[-1].total_stride

        for layer_ind, conv_filters in enumerate(
            net_params["up_conv_kernels"]
        ):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        min_pad_value = (
            self.total_stride * int(self.pad_image) if self.pad_image else 0
        )
        ...
        if self.channel_axis == 1:
            pad_y = [
                ...
                + tf.math.mod(
                    self.total_stride
                    - tf.math.mod(input_shape[3], self.total_stride),
                    self.total_stride,
                ),
            ]
            pad_x = [
                ...
                + tf.math.mod(
                    self.total_stride
                    - tf.math.mod(input_shape[4], self.total_stride),
                    self.total_stride,
                ),
            ]
            ...
            pad_y = [
                ...
                + tf.math.mod(
                    self.total_stride
                    - tf.math.mod(input_shape[2], self.total_stride),
                    self.total_stride,
                ),
            ]
            pad_x = [
        ...
        for down_layer in self.DownLayers:
            ...
            out_down, out_skip = down_layer(
                out_down, training=training, mask=mask
            )
        up_input = out_skip
        skip_inputs.reverse()
        ...
        for up_layer, skip_input in zip(self.UpLayers, skip_inputs):
            up_input = up_layer(
                (up_input, skip_input), training=training, mask=mask
            )
        logits_output_shape = up_input.shape
        logits_output = tf.reshape(
            up_input,
            [
                input_shape[0],
                input_shape[1],
                logits_output_shape[1],
                logits_output_shape[2],
                logits_output_shape[3],
            ],
        )
